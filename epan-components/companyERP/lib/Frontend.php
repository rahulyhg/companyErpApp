...
class Frontend extends ApiFrontend {
  ...
  //  $menu=$this->add('Menu',null,'Menu');
   //$menu->addMenuItem('index','Welcome');
    //$menu->addMenuItem('course','Course');
    if($this->api->auth->model['is_admin'])
            {
    $menu->addMenuItem('inventory','Inventory');
    $menu->addMenuItem('brands','Brands');
    }
    if(!$this->api->auth->model['is_admin']){        
    $menu->addMenuItem('contact','Contact');
    $menu->addMenuItem('holidaylist','holidaylist');
     }
